    ...
    public class PeriodicMaintenance
    {
        ...
        public static void Run()
        {
            ...
            try
            {
                ...
                Helpers.RunSqlStatement(Name, "update TwitterTrackFilter set Hits1d = Hits1d * 0.995198, Discards1d = Discards1d * 0.995198 where IsActive;");
                Console.Write(".");
                ...
                Helpers.RunSqlStatement(Name, "update WordScore set Score1h = Score1h * 0.890899, Score4d = Score4d * 0.99879734;");
                Console.Write(".");
                ...
                Helpers.RunSqlStatement(Name, "create temporary table tmpWordsToDelete (WordID bigint(20) unsigned not null, primary key (WordID)) select WordID from WordScore where score4d < 50 and score1h < 0.5;");
                Helpers.RunSqlStatement(Name, "delete w.* from Word w, tmpWordsToDelete wd where wd.WordID = w.WordID;"); //Trigger on Word deletes from WordScore and WordTweet
                Helpers.RunSqlStatement(Name, "drop table tmpWordsToDelete;");
                Console.Write(".");

                //Decrease user scores and delete when score is too low
                //0.995198 every 10 minutes gives 50% decrease per 12 hours
                Helpers.RunSqlStatement(Name, "update TwitterUser set Score12h = Score12h * 0.995198;");
                Helpers.RunSqlStatement(Name, "delete from TwitterUser where not exists (select 1 from Tweet where Tweet.UserID = TwitterUser.UserID limit 1) and Score12h < 0.5;");
                ...
                Helpers.RunSqlStatement(Name, "update Constants set value = (select " + ratio + " * max(score4d) from WordScore) where name = 'WordScore4dHigh';");
                Console.Write(".");
                ...
                Helpers.RunSqlStatement(Name, deleteStopWordReferencesSql);
                Console.Write(".");
