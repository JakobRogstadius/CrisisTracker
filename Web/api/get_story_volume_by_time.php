...
"select CreatedAt, sum(TweetsFromT) FirstSeenAt, sum(TweetCount) TweetCount from (
	...
	select
	  TextHash,
	  from_unixtime(dT*ceil(least(MaxTime, unix_timestamp(min(CreatedAt)))/dT)) CreatedAt,
	...
	select
	  from_unixtime(dT*ceil(least(MaxTime, unix_timestamp(CreatedAt))/dT)) CreatedAt,
	  0 as TweetsFromT,
      count(*) TweetCount
	from
		(select
