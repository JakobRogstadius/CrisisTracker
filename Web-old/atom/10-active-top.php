 ...
 *******************************************************************************/
  ...
  $FEEDURL = $SITEURL . '/atom/10-active-top.php';
  ...
  $rssfeed .= '<title>CrisisTracker RSS feed - Syrian uprising - Active stories</title>';
  $rssfeed .= '<updated>' . gmdate("c", time()) . '</updated>';
  $rssfeed .= '<subtitle>Most active stories among top users in the past four hours</subtitle>';
  ...
  $query = "select 
    ...
    from (
            ...
            order by TopUserCountRecent desc
            limit 10
        ) T
  ...
  while($row = mysql_fetch_array($result)) {
      ...
      $endTime = $row['EndTime'];
      $topUserCountRecent = $row['TopUserCountRecent'];
