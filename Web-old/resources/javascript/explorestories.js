...
function populateFilterSettings() {
  ...
  //Sortorder
    ...
    document.getElementById("sortOrderSelect").selectedIndex = index;
    switchPriority(settings["sortorder"]);
	}
	else {
    switchPriority("active-top");
	}
}
...
function callBackQueryStoriesLoaded(stories_container) {
  ...
  else //No stories matched search
  {
    $('#story-list-items').append('<li class="story-list-item"><span class="column title"><br/>No stories match your filter. Try using fewer filter criteria at the same time, or other search terms.<br /><br />If you are trying to do topic-based curation, make sure you only filter by keyword and/or time as other tags cannot automatically be generated by the system.</span></li>');
    console.log('no hits');
  }
